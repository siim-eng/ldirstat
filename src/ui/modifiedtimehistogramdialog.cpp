...
QDate dateForMinutes(std::uint32_t minutes) { return dateTimeForMinutes(minutes).date(); }

std::uint32_t defaultSliderRangeStartMinutes(const ModifiedTimeHistogramBounds &bounds, std::uint32_t nowMinutes) {
    if (!bounds.hasKnownFiles) return nowMinutes;

    const QDate nowDate = dateForMinutes(nowMinutes);
    const QDate fiveYearsBack = nowDate.addYears(-5);
    const std::uint32_t fiveYearsBackMinutes = minutesForDate(fiveYearsBack);
    if (bounds.earliestMinutes < fiveYearsBackMinutes) return fiveYearsBackMinutes;
    return bounds.earliestMinutes;
}

QString formatPrimaryMetric(const ModifiedTimeHistogramBin &bin, HistogramMetricMode mode) {
    if (mode == HistogramMetricMode::FileCount) {
        return QObject::tr("%1 files").arg(bin.fileCount);
    }
...
struct HistogramDialogState {
    ...
    std::uint32_t nowMinutes = 0;
    std::uint32_t oldestKnownMinutes = 0;
    std::uint32_t sliderRangeStartMinutes = 0;
    std::uint32_t pendingStartMinutes = 0;
    std::uint32_t appliedStartMinutes = 0;
...
struct HistogramDialogWidgets {
    ...
    QDateEdit *sliderRangeDateEdit = nullptr;
    QSlider *slider = nullptr;
    QLabel *startDateLabel = nullptr;
...
std::shared_ptr<HistogramDialogState> createHistogramDialogState(const DirEntryStore &store, EntryRef dirRef) {
    ...
    state->nowMinutes = currentEpochMinutes();
    if (state->nowMinutes == 0) state->nowMinutes = state->bounds.hasKnownFiles ? state->bounds.latestMinutes : 0;

    state->oldestKnownMinutes =
        state->bounds.hasKnownFiles ? std::min(state->bounds.earliestMinutes, state->nowMinutes) : state->nowMinutes;
    state->sliderRangeStartMinutes = defaultSliderRangeStartMinutes(state->bounds, state->nowMinutes);
    state->pendingStartMinutes = state->sliderRangeStartMinutes;
    state->appliedStartMinutes = state->sliderRangeStartMinutes;
    return state;
}
...
QLabel *createDialogTimestampRuleLabel(QWidget *parent) {
    auto *ruleLabel = new QLabel(
        QObject::tr("Timestamp rule: the histogram uses modified time, except when a file's creation time is newer than its modified time. "
...
HistogramDialogWidgets buildHistogramDialogWidgets(ModifiedTimeHistogramDialog *dialog,
    ...
    auto *sliderRangeLabel = new QLabel(QObject::tr("Slider Start"), dialog);
    controlsLayout->addWidget(sliderRangeLabel);

    widgets.sliderRangeDateEdit = new QDateEdit(dialog);
    widgets.sliderRangeDateEdit->setDisplayFormat(QStringLiteral("yyyy-MM-dd"));
    widgets.sliderRangeDateEdit->setCalendarPopup(true);
    widgets.sliderRangeDateEdit->setEnabled(state.bounds.hasKnownFiles);
    widgets.sliderRangeDateEdit->setMinimumDate(dateForMinutes(state.oldestKnownMinutes));
    widgets.sliderRangeDateEdit->setMaximumDate(dateForMinutes(state.nowMinutes));
    widgets.sliderRangeDateEdit->setDate(dateForMinutes(state.sliderRangeStartMinutes));
    widgets.sliderRangeDateEdit->setCurrentSection(QDateTimeEdit::MonthSection);

    const QString sliderRangeToolTip =
        QObject::tr("Sets the earliest date shown on the slider. Use this to ignore very old outlier files and make the slider more useful.");
    sliderRangeLabel->setToolTip(sliderRangeToolTip);
    widgets.sliderRangeDateEdit->setToolTip(sliderRangeToolTip);
    controlsLayout->addWidget(widgets.sliderRangeDateEdit);

    controlsLayout->addSpacing(12);
    controlsLayout->addWidget(new QLabel(QObject::tr("Selected Date"), dialog));

    widgets.slider = new QSlider(Qt::Horizontal, dialog);
    widgets.slider->setTracking(true);
    widgets.slider->setMinimum(sliderIntValue(state.sliderRangeStartMinutes));
    widgets.slider->setMaximum(sliderIntValue(state.nowMinutes));
    widgets.slider->setValue(sliderIntValue(state.appliedStartMinutes));
    widgets.slider->setEnabled(state.bounds.hasKnownFiles);
    controlsLayout->addWidget(widgets.slider, 1);

    widgets.startDateLabel =
        new QLabel(state.bounds.hasKnownFiles ? formatHistogramMinutes(state.appliedStartMinutes) : QObject::tr("No known timestamps"), dialog);
    widgets.startDateLabel->setMinimumWidth(dialog->fontMetrics().horizontalAdvance(QStringLiteral("0000-00-00 00:00")));
    widgets.startDateLabel->setTextInteractionFlags(Qt::TextSelectableByMouse);
    controlsLayout->addWidget(widgets.startDateLabel);
...
void syncHistogramSliderRange(const std::shared_ptr<HistogramDialogState> &state, const HistogramDialogWidgets &widgets) {
    const int clampedMaximum = sliderIntValue(state->nowMinutes);
    if (widgets.slider->maximum() != clampedMaximum) widgets.slider->setMaximum(clampedMaximum);

    widgets.sliderRangeDateEdit->setMaximumDate(dateForMinutes(state->nowMinutes));
    if (state->sliderRangeStartMinutes > state->nowMinutes) state->sliderRangeStartMinutes = state->nowMinutes;
    if (state->pendingStartMinutes < state->sliderRangeStartMinutes) state->pendingStartMinutes = state->sliderRangeStartMinutes;

    {
        const QSignalBlocker sliderBlocker(*widgets.slider);
        widgets.slider->setMinimum(sliderIntValue(state->sliderRangeStartMinutes));
        if (widgets.slider->value() < widgets.slider->minimum()) widgets.slider->setValue(widgets.slider->minimum());
    }
    {
        const QSignalBlocker dateBlocker(*widgets.sliderRangeDateEdit);
        widgets.sliderRangeDateEdit->setDate(dateForMinutes(state->sliderRangeStartMinutes));
    }
}
...
void applyHistogramDialogState(const std::shared_ptr<HistogramDialogState> &state,
    ...
    state->nowMinutes = std::max(state->nowMinutes, currentEpochMinutes());
    syncHistogramSliderRange(state, widgets);

    state->appliedStartMinutes = std::min(state->pendingStartMinutes, state->nowMinutes);
    widgets.startDateLabel->setText(state->bounds.hasKnownFiles ? formatHistogramMinutes(state->appliedStartMinutes)
                                                                : QObject::tr("No known timestamps"));

    widgets.histogramWidget->setBins(state->builder.build(dirRef, state->appliedStartMinutes, state->nowMinutes));
    ...
    widgets.summaryLabel->setText(QObject::tr("Showing 24 bins from %1 to %2 for the current directory and all subdirectories.")
                                      .arg(formatHistogramMinutes(state->appliedStartMinutes), formatHistogramMinutes(state->nowMinutes)));
}
...
void connectHistogramDialogSignals(ModifiedTimeHistogramDialog *dialog,
                                   ...
                                   EntryRef dirRef,
                                   const HistogramDialogWidgets &widgets) {
    QObject::connect(widgets.slider, &QSlider::valueChanged, dialog, [state, startDateLabel = widgets.startDateLabel, timer = widgets.debounceTimer](int value) {
        state->pendingStartMinutes = static_cast<std::uint32_t>(std::max(value, 0));
        startDateLabel->setText(formatHistogramMinutes(state->pendingStartMinutes));
        timer->start();
    });
    ...
    QObject::connect(widgets.sliderRangeDateEdit,
                     &QDateEdit::dateChanged,
                     dialog,
                     [state, slider = widgets.slider, startDateLabel = widgets.startDateLabel, timer = widgets.debounceTimer](const QDate &date) {
                         state->sliderRangeStartMinutes = minutesForDate(date);
                         state->sliderRangeStartMinutes =
                             std::clamp(state->sliderRangeStartMinutes, state->oldestKnownMinutes, state->nowMinutes);
                         if (state->pendingStartMinutes < state->sliderRangeStartMinutes) {
                             state->pendingStartMinutes = state->sliderRangeStartMinutes;
                             startDateLabel->setText(formatHistogramMinutes(state->pendingStartMinutes));
                         }
                         {
                             const QSignalBlocker blocker(*slider);
                             slider->setMinimum(sliderIntValue(state->sliderRangeStartMinutes));
                             if (slider->value() < slider->minimum()) slider->setValue(slider->minimum());
                         }
                         timer->start();
                     });
}
