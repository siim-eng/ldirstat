...
QColor headerColorForFill(const QColor &fill) {
    if (fill.lightness() < 128) return fill.lighter(120);
...
void TreeMapWidget::rebuildLayout() {
    ...
    if (renderMode_ == RenderMode::DirectoryHeaders) {
        ...
        options.minDirAreaForChildren = std::max(options.minDirAreaForChildren, options.directoryHeaderHeight * 18.0f);
        options.minDirSideForChildren = std::max(options.minDirSideForChildren, options.directoryHeaderHeight * 0.7f);
    }
...
void TreeMapWidget::paintPacked(QPainter &painter, const QPalette &widgetPalette) const {
    ...
    for (const TreeMapNode &node : treeMap_.nodes()) {
        if (node.rect.width < 1.0f || node.rect.height < 1.0f) continue;
    ...
    for (const TreeMapNode &node : treeMap_.nodes()) {
        ...
        if (labelRect.width() < 40.0f || labelRect.height() < 16.0f) continue;
    ...
    if (selectedEntry_.valid()) {
        ...
        for (const TreeMapNode &node : treeMap_.nodes()) {
            if (node.ref != selectedEntry_ || node.rect.width < 1.0f || node.rect.height < 1.0f) continue;
            const QRectF outline = selectionOutlineRect(node.rect);
            if (!outline.isValid() || outline.isEmpty()) continue;
...
void TreeMapWidget::paintDirectoryHeaders(QPainter &painter, const QPalette &widgetPalette) const {
    ...
    for (const TreeMapNode &node : treeMap_.nodes()) {
        ...
        if (entry.isDir() && hasHeader(node)) {
            const QRectF headerRect(node.rect.x, node.rect.y, node.rect.width, node.contentRect.y - node.rect.y);
    ...
    for (const TreeMapNode &node : treeMap_.nodes()) {
        ...
        if (hasHeader(node)) {
            painter.drawLine(QPointF(node.rect.x, node.contentRect.y), QPointF(node.rect.right(), node.contentRect.y));
        }
    }
    ...
    for (const TreeMapNode &node : treeMap_.nodes()) {
        ...
        QRectF labelRect;
        if (entry.isDir() && hasHeader(node)) {
            labelRect = QRectF(node.rect.x, node.rect.y, node.rect.width, node.contentRect.y - node.rect.y);
        } else {
            labelRect = toQRect(node.rect);
        }

        if (labelRect.width() < 40.0 || labelRect.height() < 16.0) continue;

        const QString text =
            metrics.elidedText(entryName(*names_, entry), Qt::ElideRight, static_cast<int>(labelRect.width()) - 6);
        if (text.isEmpty()) continue;
