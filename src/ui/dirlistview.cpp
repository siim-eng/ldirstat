...
DirListView::DirListView(QWidget *parent)
    ...
    connect(scrollArea_->horizontalScrollBar(), &QScrollBar::rangeChanged, this, [this]() { syncColumnHeights(); });
}

std::vector<EntryRef> DirListView::selectedEntries() const {
    if (activeColumnIndex_ < 0 || activeColumnIndex_ >= static_cast<int>(columns_.size())) return {};
...
bool DirListView::handleArrowKey(int key, Qt::KeyboardModifiers modifiers) {
    ...
    if ((modifiers & ~supportedModifiers) != Qt::NoModifier) return false;
    ...
    if (activeColumnIndex_ < 0 || activeColumnIndex_ >= static_cast<int>(columns_.size())) activeColumnIndex_ = 0;
    ...
    if (key == Qt::Key_Left) {
        ...
        if (focusedColumnIndex < static_cast<int>(columns_.size())) columns_[focusedColumnIndex]->clearFocus();
        rootFocused_ = false;
    ...
    if (column->rowCount() == 0) return true;
    ...
    applyFocusInColumn(activeColumnIndex_,
                       focusedRow,
                       modifiers,
                       modifiers.testFlag(Qt::ControlModifier) && !modifiers.testFlag(Qt::ShiftModifier));
    return true;
...
void DirListView::setRoot(const DirEntryStore &store, const NameStore &names, EntryRef root) {
    store_ = &store;
    names_ = &names;
...
void DirListView::selectEntry(EntryRef ref) {
    ...
    for (EntryRef pathRef : path) {
        ...
        if (entry.isDir()) addColumn(pathRef);
    }
...
void DirListView::onColumnFocusChanged(EntryRef ref, bool isDir) {
    if (suppressColumnFocusChanges_) return;
    ...
    if (focusWidget == nullptr || !col->isAncestorOf(focusWidget)) setFocus(Qt::MouseFocusReason);
    setActiveColumnIndex(colIndex);
...
void DirListView::applyFocusInColumn(int columnIndex,
    ...
    if (!column || rowIndex < 0 || rowIndex >= column->rowCount()) return;
...
void DirListView::addColumn(EntryRef dirRef) {
    auto *col = new DirListColumn(*store_, *names_, dirRef, rootSize_, themeColors_, columnWidth_, scrollContent_);

    const int insertIndex = columnsLayout_->count() - 1;
    columnsLayout_->insertWidget(insertIndex, col);
    columns_.push_back(col);

    connect(col, &DirListColumn::activated, this, &DirListView::onColumnActivated);
    connect(col, &DirListColumn::focusChanged, this, &DirListView::onColumnFocusChanged);
    connect(col, &DirListColumn::contextMenuRequested, this, &DirListView::onColumnContextMenuRequested);
...
void DirListView::syncPathHighlights() {
    for (int i = 0; i < static_cast<int>(columns_.size()); ++i) {
        const EntryRef nextDir = i + 1 < static_cast<int>(columns_.size()) ? columns_[i + 1]->dirRef() : kNoEntry;
...
void DirListView::truncateColumnsAfter(int columnIndex) {
    ...
    if (startRemove < static_cast<int>(columns_.size())) columns_.erase(columns_.begin() + startRemove, columns_.end());

    if (columns_.empty()) {
        activeColumnIndex_ = -1;
        rootFocused_ = true;
    } else {
        activeColumnIndex_ = std::clamp(activeColumnIndex_, 0, static_cast<int>(columns_.size()) - 1);
    }
...
void DirListView::scrollToLastColumn() {
    QTimer::singleShot(0, this, [this]() {
        if (!columns_.empty()) scrollArea_->ensureWidgetVisible(columns_.back());
    });
}
