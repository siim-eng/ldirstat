...
inline QString entryFullPath(const DirEntryStore& store, const NameStore& names,
                             EntryRef ref) {
...
inline QString entryTooltip(const DirEntryStore& store, const NameStore& names,
                            EntryRef ref) {
    ...
    if (entry.isFile()) {
        ...
        if (entry.hardLinks > 1) {
            tip += '\n' + QString::number(entry.hardLinks) + ' '
