...
std::string decodeProcMountField(std::string_view input) {
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        if (input[i] == '\\' && i + 3 < input.size() && isOctalDigit(input[i + 1]) && isOctalDigit(input[i + 2])
            && isOctalDigit(input[i + 3])) {
            const int value = ((input[i + 1] - '0') << 6) | ((input[i + 2] - '0') << 3) | (input[i + 3] - '0');
...
std::string decodeLsblkValue(std::string_view input) {
    ...
    for (size_t i = 0; i < input.size(); ++i) {
        ...
        case '"': output.push_back('"'); break;
        default: output.push_back(escape); break;
        }
    }
...
std::vector<std::string> splitMountPoints(std::string_view mountPoints) {
    ...
    while (start <= mountPoints.size()) {
        ...
        const std::string_view item =
            end == std::string_view::npos ? mountPoints.substr(start) : mountPoints.substr(start, end - start);

        if (!item.empty() && item != "[SWAP]") result.emplace_back(item);
...
bool parseLsblkLine(const std::string &line, std::unordered_map<std::string, std::string> &fields) {
    fields.clear();
    ...
    while (pos < line.size()) {
        ...
        while (pos < line.size() && line[pos] != '=')
            ++pos;
        if (pos == line.size() || keyStart == pos) return false;
        ...
        if (!foundClosingQuote) return false;
...
std::vector<MountInfo> readProcMounts() {
    ...
    while (std::getline(file, line)) {
        ...
        struct stat st{};
        if (stat(mountPoint.c_str(), &st) != 0) continue;
        ...
        if (info.kind == FileSystemType::Real || info.kind == FileSystemType::Network) {
            struct statvfs svfs{};
...
const MountInfo *findBestMountedLocal(const std::vector<MountInfo> &mounts, const std::string &devicePath) {
    const MountInfo *best = nullptr;

    for (const auto &mount : mounts) {
        if (mount.kind != FileSystemType::Real || mount.device != devicePath) continue;

        if (!best || betterMountPoint(mount.mountPoint, best->mountPoint)) best = &mount;
    }
...
void appendMountedLocalFallbackVolumes(const std::vector<MountInfo> &mounts, std::vector<VolumeInfo> &volumes) {
    std::unordered_map<std::string, size_t> byDevice;

    for (const auto &mount : mounts) {
        if (mount.kind != FileSystemType::Real) continue;
...
void mergeLocalVolume(VolumeInfo &dst, const VolumeInfo &src) {
    if (!src.mountPoint.empty() && betterMountPoint(src.mountPoint, dst.mountPoint)) dst.mountPoint = src.mountPoint;

    if (!src.fsType.empty()) dst.fsType = src.fsType;
    if (!src.label.empty()) dst.label = src.label;
    if (!src.uuid.empty()) dst.uuid = src.uuid;

    dst.kind = src.kind;
    dst.sizeBytes = std::max(dst.sizeBytes, src.sizeBytes);
    if (src.totalBytes > 0) dst.totalBytes = src.totalBytes;
    if (src.availBytes > 0 || src.totalBytes > 0) dst.availBytes = src.availBytes;
...
bool appendLsblkVolumes(const std::vector<MountInfo> &mounts, std::vector<VolumeInfo> &volumes) {
    FILE *pipe = popen("lsblk -P -b -o PATH,TYPE,FSTYPE,LABEL,UUID,MOUNTPOINTS,SIZE,RM,HOTPLUG,RO", "r");
    ...
    auto processLine = [&](const std::string &line) {
        ...
        for (const std::string &mountPoint : splitMountPoints(fields["MOUNTPOINTS"])) {
            volume.mounted = true;
            if (betterMountPoint(mountPoint, volume.mountPoint)) volume.mountPoint = mountPoint;
        }
    ...
    if (!pendingLine.empty()) processLine(pendingLine);
...
void FileSystems::refresh() {
    ...
    for (const auto &mount : mounts_) {
        if (mount.kind != FileSystemType::Network) continue;
...
const VolumeInfo *FileSystems::findVolumeByDevice(std::string_view devicePath) const {
    for (const auto &volume : volumes_) {
        if (volume.devicePath == devicePath) return &volume;
    }
    return nullptr;
}
